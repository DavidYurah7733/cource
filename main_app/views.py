...
def generate_pdf(response, news_id):
    news = News.objects.get(pk=news_id)
    document_pdf = ap.Document()

    page = document_pdf.pages.add()

    title = ap.text.TextFragment(news.title)
    title.text_state.font_size = 20
    title.horizontal_alignment = ap.HorizontalAlignment.CENTER
    text = ap.text.TextFragment(news.content)

    page.paragraphs.add(title)
    page.paragraphs.add(text)

    image_stream = open(news.photo.path, 'rb')
    image_rect = ap.Rectangle(100, 500, 400, 400, True)

    page.add_image(image_stream, image_rect)

    document_pdf.save("output.pdf")

    with open("output.pdf", "rb") as file:
        response = HttpResponse(file.read(), content_type="application/pdf")
        response["Content-Disposition"] = f'attachment; filename="{news.title}.pdf"'
        return response


def generate_pdf_view(request, news_id):
    pdf_response = generate_pdf(request, news_id)
    return pdf_response
